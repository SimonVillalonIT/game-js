...
export async function preloadImages(obj: {
    [key: string]: string[];
}): Promise<void> {
...
export function swordCollisions(
    sword: Sword,
    enemy: GroundEnemy | FlyingEnemy,
) {
    return (
        enemy.x < sword.x + sword.width &&
...
export function playerCollisions(
    ...
    if (
        player.currentState instanceof Attack ||
        player.currentState instanceof JumpAttack
    ) {
        return (
            enemy.x < player.x + player.width - 100 &&
...
export function movementLogic(player: Player, input: InputType[]) {
    if (player.onGround()) {
        ...
        if (input.includes(" ")) {
            ...
            input.length === 0 &&
            player.currentState instanceof Walk !== true
        ) {
