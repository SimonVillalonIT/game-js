  ...
  update() {
    ...
    if (!this.stop) {
      ...
      this.currentFrameIndex =
        (this.currentFrameIndex + this.animationSpeed) %
        this.spriteImages.length;
      if (this.markedForDeletetion) this.speedX = 0;
      this.speedY = 0;
...
export class FlyingEnemy extends Enemy {
  ...
  update() {
    ...
    this.spriteImages = !this.isDead
      ? FLYING_ENEMIES_URLS.idle.map(loadStateImages)
      : FLYING_ENEMIES_URLS.dead.map(loadStateImages);
    if (!this.stop) {
      super.update();
...
export class GroundEnemy extends Enemy {
  ...
  update() {
    this.spriteImages = !this.isDead
      ? GROUND_ENEMY_URLS.idle.map(loadStateImages)
      : GROUND_ENEMY_URLS.dead.map(loadStateImages);
    super.update();
  }
}
