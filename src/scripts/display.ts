...
export class Display {
    ...
    pauseGame() {
        const duration = `${fromMsToMMSS((Date.now() as number) - (this.game.startTime as number))}`;
    ...
    gameOver() {
        const duration = `${fromMsToMMSS((this.game.endTime as number) - (this.game.startTime as number))}`;
