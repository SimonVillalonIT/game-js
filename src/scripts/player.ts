...
export default class Player {
  ...
  constructor(game: Game) {
    ...
    this.stateMap = {
      ...
      jumpAttack: new JumpAttack(this),
      dead: new Dead(this),
    };
  ...
  changeState(newState: State) {
    if (
      this.currentState instanceof Dead === false &&
  ...
  draw(context: CanvasRenderingContext2D) {
    if (
      ...
      (this.currentState instanceof Attack ||
        this.currentState instanceof JumpAttack)
    ) {
      context.strokeRect(this.x, this.y, this.width - 100, this.height);
    } else if (this.game.debug)
      context.strokeRect(this.x + 60, this.y, this.width - 70, this.height);
  ...
  getHit() {
    ...
    if (this.game.isRunning()) {
      // Only set timeout if the game is not paused
  ...
  checkCollisions() {
    this.game.enemies.forEach((enemy) => {
      ...
      if (
        swordCollisions(this.sword, enemy) &&
        !enemy.isDead &&
        (this.currentState instanceof JumpAttack ||
          this.currentState instanceof Attack)
      ) {
